# -*- coding: utf-8 -*-
"""
集中配置文件：ChiNesE 数据集 BERT-GlobalPointer 基线版本

本版本基于当前旧版 GlobalPointer 训练代码生成：
- 数据集切换为 ChiNesE；
- 实体类别数改为 10；
- 输出目录改为 outputs_ChiNesE，避免覆盖 CMeEE-V2 / CLUENER 结果；
- 使用 bert-base-chinese 作为 encoder，保留 GlobalPointer 主体、BiGRU 开关、对抗训练开关和逐类别指标输出；
...
# 输出路径
# =========================
OUTPUT_DIR = os.path.join(PROJECT_DIR, 'outputs_ChiNesE_bert_globalpointer')
...
# BERT-GlobalPointer 纯基线：关闭 BiGRU。若设为 True，则是 BERT-BiGRU-GlobalPointer，不再是纯 BERT-GlobalPointer。
USE_BIGRU = False

# 对抗训练配置
# BERT-GlobalPointer 纯基线：不使用 FGM/PGD。若启用，则是带对抗训练的 BERT-GlobalPointer 变体。
