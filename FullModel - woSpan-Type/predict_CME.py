...
if __name__ == '__main__':
    ...
    for i in range(min(3, len(data))):
        ...
        ents = predict_one(data[i]['text'], tokenizer, model, thresholds, gamma, max_len=MAX_LEN, debug=True)
        print(f"Result: {ents}")
        print("-" * 30)

    for d in tqdm(data, desc='Predicting full dataset'):
        ents = predict_one(d['text'], tokenizer, model, thresholds, gamma, max_len=MAX_LEN, debug=False)
        item = dict(d)
        item['entities'] = ents
        results.append(item)
