...
def load_json_or_jsonl(path: str):
    """读取 JSON 数组或 JSONL（每行一个 JSON 对象）格式的数据。"""
    ...
    if content.startswith('['):
        ...
        if not isinstance(data, list):
            raise ValueError(f"Expected a JSON array in {path}, but got {type(data).__name__}.")
        return data
    ...
    for line_no, line in enumerate(content.splitlines(), start=1):
        ...
        try:
            ...
            raise json.JSONDecodeError(
                f"Invalid JSONL record at line {line_no}: {exc.msg}",
                exc.doc,
                exc.pos,
            ) from exc
        if not isinstance(record, dict):
            raise ValueError(
                f"Expected a JSON object at line {line_no} in {path}, "
