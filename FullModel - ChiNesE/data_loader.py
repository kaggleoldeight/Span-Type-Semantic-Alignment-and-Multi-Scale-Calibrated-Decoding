...
- 支持 CMeEE-V2 的 entities 列表格式；
- 支持 CLUENER 与 ChiNesE/new_* 的 JSONL label 字典格式；
- ChiNesE/new_* 的 span 为闭区间 [start, end]，可直接保留嵌套实体。
"""
import json
...
# ChiNesE/new_* 数据集标签。样例中 span 为字符级闭区间 [start, end]，并允许重叠/嵌套实体。
CHINESE_ENT2ID = {
    "Person": 0,
    "Location": 1,
    "Organization": 2,
...
def _select_ent2id() -> Dict[str, int]:
    ...
    if 'chinese' in name or 'nested' in name or 'new' in name:
        return CHINESE_ENT2ID.copy()
...
def get_label_descriptions() -> Dict[str, str]:
    ...
    if 'chinese' in name or 'nested' in name or 'new' in name:
        return {k: CHINESE_LABEL_DESCRIPTIONS.get(k, k) for k in ent2id.keys()}
    return {k: CME_LABEL_DESCRIPTIONS.get(k, k) for k in ent2id.keys()}


def _resolve_existing_path(path: str) -> str:
    """兼容 Windows 隐藏扩展名导致的 new_train / new_train.json 命名差异。"""
    import os
...
def load_data(path: str):
    ...
    3) ChiNesE/new_*: {'id': int, 'text': str, 'label': {type: {mention: [[start,end], ...]}}}
       其中 start/end 为字符级闭区间，天然支持嵌套/重叠实体。
    4) 通用 entities: start/end/type 或 start_idx/end_idx/type 字段。
    """
    D = []
